...
# Streamlit Title
st.title("Auto-Aligned Map with Pipes and Landmarks")
...
if csv_file and screenshot_file:
    ...
    # Draw Pipes and Landmarks
    for _, row in valid_data.iterrows():
        coords = row["Coordinates"]
        normalized_coords = normalize_coordinates(coords, x_min, x_max, y_min, y_max)

        if row["Length (meters)"] > 0:  # Pipes
            x, y = zip(*normalized_coords)
